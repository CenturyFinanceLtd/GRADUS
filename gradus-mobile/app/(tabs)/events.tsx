...
import { fetchEvents, EventItem } from "@/services/events";
...
export default function EventsScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const data = await fetchEvents(timeframe, 20);
        setItems(
          data.filter(
        ...
        console.warn("[events] Failed to load", error);
      } finally {
        setLoading(false);
      }
    };
          ...
          <FlatList
            ...
            ListHeaderComponent={
              <View style={styles.header}>
                <Text style={styles.heading}>Events & meetups</Text>
                <Text style={styles.subheading}>
                  Community sessions, announcements, and special gatherings.
                </Text>
                <ScrollView
                  horizontal
            ...
            ListEmptyComponent={
              <Text style={styles.empty}>No events published yet.</Text>
            }
            renderItem={({ item }) => (
              <Pressable
                style={styles.card}
