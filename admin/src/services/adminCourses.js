...
export const updateAdminCourse = async ({ id, data: payload, token }) => {
  const data = await apiClient(`/admin/courses/${id}`, {
    method: "PATCH",
    data: payload,
    token,
  });
...
export const upsertRawCourse = async ({ data: payload, token }) => {
  const data = await apiClient("/admin/courses/raw", {
    method: "POST",
    data: payload,
    token,
  });
...
export const deleteCourseBySlug = async ({ slug, token }) => {
  const data = await apiClient(
    `/admin/courses/slug/${encodeURIComponent(slug)}`,
    { method: "DELETE", token }
  );
  return data?.message === "Course deleted";
};

export const fetchCourseProgressAdmin = async ({
  slug,
  token,
  userId,
} = {}) => {
  ...
  const data = await apiClient(`/admin/courses/progress?${params.toString()}`, {
    token,
  });
  return {
