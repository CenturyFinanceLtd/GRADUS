...
const createExpertVideo = asyncHandler(async (req, res) => {
  ...
  let uploadResult;
  try {
    uploadResult = await uploadToCloudinary(req.file.path, { folder: expertVideosFolder });
  } catch (error) {
    ...
    if (status === 413) {
      ...
      throw new Error(`Video is too large. Maximum allowed size is ${limitText}. If the file fits, increase the Cloudinary plan or reverse proxy body limit.`);
    }
