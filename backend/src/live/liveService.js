...
const generateLiveKitToken = async (
  ...
  at.addGrant({
    roomJoin: true,
    room: roomId,
    canPublish: true, // Allow everyone to publish initially (controlled via client/admin)
    canSubscribe: true,
    canPublishData: true,
  });
...
const normalizeIsoDate = (value) => {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new LiveSessionError(
      "Invalid date supplied for scheduled time.",
      400
    );
  }
...
const createSession = async ({
  ...
  if (!courseId) {
    throw new LiveSessionError(
      "Selecting a course is required to start a live class.",
      422
    );
  }

  const normalizedCourseName =
    (courseName && courseName.trim()) || "Live Course";
...
const updateSession = async (sessionId, payload = {}) => {
  ...
  if (payload.scheduledFor !== undefined) {
    updates.scheduledFor = payload.scheduledFor
      ? normalizeIsoDate(payload.scheduledFor)
      : null;
  }
...
const enforceAccessControls = (session, { passcode, meetingToken }) => {
  if (session.locked) {
    throw new LiveSessionError(
      "This live class is locked by the instructor.",
      403
    );
  }
  ...
  if (session.passcodeHash) {
    const providedHash =
      meetingToken && meetingToken === session.meetingToken
...
const registerStudentParticipant = async ({
  sessionId,
  user,
  displayName,
  passcode,
  ...
  const { session: updatedSession, participant } = await addParticipantRecord(
    sessionId,
    {
      role: "student",
  ...
  return {
    session: await buildSessionSnapshotWithParticipants(updatedSession, {
      includeRooms: true,
    }),
    participant: toParticipantSnapshot(participant, {
      includeSignalingKey: true,
    }),
...
const registerInstructorParticipant = async ({
  sessionId,
  admin,
  hostSecret,
}) => {
...
const commandParticipantMedia = async ({
  sessionId,
  participantId,
  admin,
  media,
...
const moveParticipantRoom = async ({
  sessionId,
  participantId,
  admin,
  roomId,
  ...
  liveEvents.emit("participant-room-changed", {
    sessionId,
    participantId,
    roomId: roomId || null,
  });
...
const saveRecording = async ({
  ...
  try {
    ...
    throw new LiveSessionError(
      error?.message || "Failed to upload recording.",
      500
    );
  }
