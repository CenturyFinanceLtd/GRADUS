...
$ { portalName || 'Gradus Admin Portal' }: Approval requested
for $ { requester.fullName }
...
Approve as $ { option.label }: $ { option.url }
`)\n    .join('\\n');\n\n  const text = `
...
n\\ n ` +\n    `
$ { approvalText }\\
nReject: $ { rejectionUrl }
`;\n\n  const approvalButtonsHtml = approvalOptions\n    .map((option) => `\
n < a href = "${option.url}"
style = "background: #0B5394; color: #fff; padding: 10px 16px; text-decoration: none; border-radius: 4px; display: inline-block; margin-right: 12px; margin-bottom: 12px;" > Approve as $ { option.label } < /a>\n      `)\n    .join('');\n\n  const html = `\n    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">\n      <h2 style="color: #0B5394; margin-bottom: 12px;">${portalName || 'Gradus Admin Portal'} - Approval Needed</h
...
style = "background: #d9534f; color: #fff; padding: 10px 16px; text-decoration: none; border-radius: 4px; display: inline-block;" > Reject < /a>\n      <p style="margin-top: 20px;">If the buttons do not work, you can copy and paste these links into your browser:</p > \n $ { approvalOptions\ n.map((option) => `<p style="margin: 0; font-size: 14px;">Approve as ${option.label}: <a href="${option.url}">${option.url}</a></p>`)\ n.join('') }\
n < p style = "margin: 0; font-size: 14px;" > Reject: < a href = "${rejectionUrl}" > $ { rejectionUrl } < /a></p > \n < /div>\n  `;\n\n  return sendEmail({ to, subject, text, html });\n}; \n\n`;
