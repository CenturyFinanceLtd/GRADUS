...
- Dedicated mailbox credentials per template (optional): `SMTP_VERIFICATION_LOGIN_USER` / `SMTP_VERIFICATION_PASS` / `SMTP_VERIFICATION_WORKSPACE_IMPERSONATE_EMAIL` for authentication mails, and `SMTP_REGISTRATION_LOGIN_USER` / `SMTP_REGISTRATION_PASS` / `SMTP_REGISTRATION_WORKSPACE_IMPERSONATE_EMAIL` for registration mails. When provided, those credentials are used only for the matching template; otherwise the default SMTP login is reused.
