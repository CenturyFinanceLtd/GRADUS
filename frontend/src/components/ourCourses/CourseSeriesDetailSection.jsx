...
const CourseSeriesDetailSection = ({ course, isAltBackground = false }) => {
  ...
  const certificationCount = course.certifications?.length || 0;
  ...
  const hasOverflowWeeks = course.weeks.length > VISIBLE_WEEKS;
  ...
  }, [measureCollapsedHeight, course.id, course.weeks.length]);
  ...
  weekCardRefs.current.length = course.weeks.length;
  ...
  return (
    <section id={course.id} className={sectionClass}>
        ...
        <div className='row gy-5 align-items-start'>
          <div className='col-lg-4'>
            <div className='pe-xl-5'>
              ...
              <h2 className='mb-16 text-neutral-900'>{course.name} Learning Journey</h2>
              <p className='text-neutral-500 mb-24'>{course.focus}</p>
              <div className='mb-24'>
                ...
                <ul className='list-unstyled d-grid gap-12 mb-0'>
                  {course.approvals.map((item, approvalIndex) => (
                    <li
                      className='d-flex align-items-start gap-12 text-neutral-600'
                      key={`course-${course.id}-approval-${approvalIndex}`}
                    >
              ...
              <div className='p-24 rounded-20 bg-main-25 border border-main-100 mb-24'>
                ...
                <p className='text-neutral-600 mb-0'>{course.placementRange}</p>
              </div>
              {course.outcomeSummary ? (
                <div className='p-24 rounded-20 bg-white border border-neutral-30'>
                  ...
                  <p className='text-neutral-600 mb-0'>{course.outcomeSummary}</p>
                </div>
              ) : null}
            </div>
          </div>
                ...
                {course.weeks.map((week, weekIndex) => {
                  ...
                  return (
                    <div
                      ...
                      key={`course-${course.id}-week-${weekIndex}`}
                    >
              ...
              {hasOverflowWeeks ? (
                <div className='text-center mt-28'>
                  <button
                    ...
                    aria-controls={`${course.id}-week-list`}
                  >
                    {showAllWeeks ? 'View Less Weeks' : `View All ${course.weeks.length} Weeks`}
        ...
        <div className='mt-40'>
          {course.deliverables?.length ? (
            <div className='rounded-24 border border-neutral-30 p-32 bg-white mb-32'>
              ...
              <ul className='list-unstyled d-grid gap-12 mb-0'>
                {course.deliverables.map((item, itemIndex) => (
                  <li
                    className='d-flex align-items-start gap-12 text-neutral-600'
                    key={`course-${course.id}-deliverable-${itemIndex}`}
                  >
          ...
          {course.outcomes?.length ? (
            <div className='rounded-24 border border-neutral-30 p-32 bg-main-25 mb-32'>
              ...
              <ul className='list-unstyled d-grid gap-12 mb-0'>
                {course.outcomes.map((item, outcomeIndex) => (
                  <li
                    className='d-flex align-items-start gap-12 text-neutral-600'
                    key={`course-${course.id}-outcome-${outcomeIndex}`}
                  >
            ...
            <div className='rounded-24 border border-neutral-30 p-32 bg-white mb-32'>
              ...
              <div className='row gy-4 d-none d-lg-flex'>
                {course.certifications.map((cert, certIndex) => (
                  <div className='col-xl-4 col-md-6' key={`course-${course.id}-cert-${certIndex}`}>
              ...
              <div className='d-lg-none'>
                <Slider {...mobileCertificationSettings} className='our-courses-cert-slider'>
                  {course.certifications.map((cert, certIndex) => (
                    <div className='px-1' key={`course-${course.id}-cert-slide-${certIndex}`}>
                      {renderCertificationCard(cert, certIndex)}
                    </div>
                  ))}
                </Slider>
          ...
          {course.finalAward ? (
            <div className='rounded-24 border border-main-100 bg-main-600 p-32 text-white mb-32'>
              ...
              <p className='mb-0'>{course.finalAward}</p>
            </div>
          ) : null}
          ...
          <div className='rounded-24 border border-neutral-30 p-32 bg-white'>
            <div className='d-flex flex-wrap justify-content-between align-items-start gap-16 mb-24'>
              ...
              <span className='text-neutral-500 text-md'>
                Guaranteed access to {course.partners.length} leading organizations
              </span>
            </div>
            <PlacementPartnersCarousel
              partners={course.partners}
              carouselId={`${course.id}-partners`}
            />
          </div>
        </div>
      </div>
